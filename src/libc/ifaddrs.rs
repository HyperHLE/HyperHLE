...
fn getifaddrs(env: &mut Environment, ifap: MutPtr<MutPtr<ifaddrs>>) -> i32 {
    ...
    // Allocate a single ifaddrs struct in guest memory
    let name_str = "lo0\0";
    let name_ptr: MutPtr<u8> = env.mem.guest_alloc(name_str.len() as u32);
    env.mem.write_bytes(name_ptr, name_str.as_bytes());

    let ifa_ptr: MutPtr<ifaddrs> = env.mem.guest_alloc(std::mem::size_of::<ifaddrs>() as u32);
...
fn freeifaddrs(_env: &mut Environment, _ifa: MutPtr<ifaddrs>) {
    // In a stub, we can leak this small amount of guest memory to avoid complexity.
}
...
fn if_nametoindex(env: &mut Environment, ifname: ConstPtr<u8>) -> u32 {
    ...
    if name == "en0" || name == "en1" || name == "lo0" {
        return 1; // Return a valid index instead of 0/error
    }
    set_errno(env, ENXIO);
    0
}
...
fn if_indextoname(_env: &mut Environment, ifindex: u32, ifname: MutPtr<u8>) -> MutPtr<u8> {
    if ifindex == 1 {
        // Just return lo0 if they ask for index 1
        return ifname; 
    }
    MutPtr::null()
}
